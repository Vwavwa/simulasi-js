...
function virusCheckRecursive (str, viruses) {
  ...
  var nextStr = str.substr(1)
  for(var x=0; x<virus.length; x++){
    if(str[0].toLowerCase() === virus[x].toLowerCase()){
      return 1 + virusCheckRecursive(nextStr, viruses)
    }
  }
  return virusCheckRecursive(nextStr, viruses)
}
